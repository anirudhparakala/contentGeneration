...
from .models import (
    ...
    parse_json_text,
    parse_selected_row,
    preprocess_evidence_snippets,
    source_type_to_platform_hint,
    to_utc_z,
    utc_now,
    validate_extract_payload,
    validate_score_payload,
)
from .prompts import PromptLoadError, PromptRenderError, load_prompt_templates, render_prompt
from .state import IntelligenceStore, StateError


LOGGER = logging.getLogger(__name__)


class FatalIntelligenceError(RuntimeError):
    pass
...
@dataclass(frozen=True)
class Stage5IntelligenceConfig:
...
class PipelineConfig:
    paths: PathsConfig
    llm: LLMConfig
    stage_5_intelligence: Stage5IntelligenceConfig
    ...
    *,
    ...
    try:
        ...
        max_items = (
            ...
            if max_items_override_value is not None
            else pipeline.stage_5_intelligence.max_items_default
        )
        ...
        with output_file.open("w", encoding="utf-8", newline="\n") as out_handle:
            ...
            LOGGER.info(
                "stage_5_intelligence start run_id=%s db_path=%s output_path=%s report_path=%s max_items=%s llm_provider=%s llm_model=%s api_key_source=%s",
                run_id,
                db_path,
                output_file,
                report_file,
            ...
            try:
                store.validate_enriched_items_table()
                store.ensure_ideas_table()
                store.validate_ideas_compatibility()
                ...
                for raw in selected_rows:
                    _process_selected_row(
                        ...
                        input_max_chars=pipeline.stage_5_intelligence.input_max_chars,
                    )
            finally:
                store.close()
        ...
        if counters["failed_count"] > 0:
            LOGGER.warning(
                "stage_5_intelligence fail_breakdown=%s",
                json.dumps(fail_breakdown, ensure_ascii=True),
            )
        LOGGER.info(
            "stage_5_intelligence complete run_id=%s items_available_total=%s selected_rows_total=%s items_selected=%s success_count=%s failed_count=%s inserted_db=%s skipped_already_present=%s",
            run_id,
        ...
        if report_file is not None:
            ...
            try:
                ...
                raise FatalIntelligenceError(
                    f"{fatal_error}; failed writing fatal report: {report_exc}"
                ) from report_exc

        raise FatalIntelligenceError(fatal_error) from exc
...
def _process_selected_row(
    *,
    ...
    try:
        ...
        platform_hint = source_type_to_platform_hint(row.source_type)
        ...
        extract_prompt = render_prompt(
            prompts.extract,
            {
                ...
                "URL": row.url,
                "CONTENT": row.enriched_text[:input_max_chars],
            },
        )

        phase = "extract_started"
        extract_response = llm_client.call_json(
            prompt=extract_prompt,
            schema_name="stage_5_extract",
            schema=_extract_schema(),
            call_label="extract",
        )
        try:
            extract_raw = parse_json_text(extract_response)
        except ValueError:
            _record_failure(counters, fail_breakdown, "extract_invalid_json")
            return
        try:
            extract_payload = validate_extract_payload(extract_raw)
        except ExtractValidationError:
            _record_failure(counters, fail_breakdown, "extract_validation_failed")
            return

        phase = "score_started"
        evidence = preprocess_evidence_snippets(row.evidence_snippets_raw)
        score_prompt = render_prompt(
            prompts.score,
            {
                "PLATFORM_HINT": platform_hint,
                "TITLE": row.title,
                "TOPIC": extract_payload.topic,
                "CORE_CLAIM": extract_payload.core_claim,
                "WORKFLOW_STEPS": json.dumps(extract_payload.workflow_steps, ensure_ascii=True),
                "TOOLS_MENTIONED": json.dumps(extract_payload.tools_mentioned, ensure_ascii=True),
                "MONETIZATION_ANGLE": extract_payload.monetization_angle,
                "METRICS_CLAIMS": json.dumps(extract_payload.metrics_claims, ensure_ascii=True),
                "ASSUMPTIONS": json.dumps(extract_payload.assumptions, ensure_ascii=True),
                "CONTENT_TYPE": extract_payload.content_type,
                "EVIDENCE_SNIPPETS": json.dumps(evidence, ensure_ascii=True),
            },
        )

        score_response = llm_client.call_json(
            prompt=score_prompt,
            schema_name="stage_5_score",
            schema=_score_schema(),
            call_label="score",
        )
        try:
            score_raw = parse_json_text(score_response)
        except ValueError:
            _record_failure(counters, fail_breakdown, "score_invalid_json")
            return
        try:
            score_payload = validate_score_payload(score_raw, platform_hint=platform_hint)
        except ScoreValidationError:
            _record_failure(counters, fail_breakdown, "score_validation_failed")
            return

        created_at = to_utc_z(utc_now())
        idea = build_idea_record(
            row=row,
        ...
        inserted = store.insert_idea(idea)
        counters["items_selected"] += 1
        counters["success_count"] += 1
        if inserted:
            counters["inserted_db"] += 1
            out_handle.write(json.dumps(idea.to_dict(), ensure_ascii=True))
            out_handle.write("\n")
        else:
        ...
        reason = "score_llm_failed" if phase == "score_started" else "extract_llm_failed"
        _record_failure(counters, fail_breakdown, reason)
    except RetryableLLMError:
        reason = "score_llm_failed" if phase == "score_started" else "extract_llm_failed"
        _record_failure(counters, fail_breakdown, reason)
    except Exception:
        if phase == "pre_extract":
            reason = "extract_validation_failed"
        elif phase == "extract_started":
            reason = "extract_llm_failed"
        else:
            reason = "score_llm_failed"
        _record_failure(counters, fail_breakdown, reason)
...
def _extract_schema() -> dict[str, Any]:
    return {
        ...
        "properties": {
            ...
            "metrics_claims": {"type": "array", "items": {"type": "string"}},
            "assumptions": {"type": "array", "items": {"type": "string"}},
            "content_type": {
                "type": "string",
                "enum": ["howto", "case_study", "tool_review", "opinion", "news", "other"],
            },
        },
    }
...
def _score_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["viral_rating", "rating_rationale", "hooks", "platform", "recommended_format"],
        "properties": {
            "viral_rating": {"type": "integer", "minimum": 1, "maximum": 10},
            ...
            "platform": {"type": "string", "enum": ["youtube", "newsletter"]},
            "recommended_format": {
                "type": "string",
                "enum": ["shorts", "tweet", "linkedin", "reel", "thread", "other"],
            },
        },
    }
...
def _resolve_output_path(*, outputs_dir: Path, date: Any) -> Path:
    return outputs_dir / f"ideas_{date.strftime('%Y-%m-%d')}.jsonl"


def _resolve_report_path(*, outputs_dir: Path, date: Any) -> Path:
    return outputs_dir / f"stage_5_report_{date.strftime('%Y-%m-%d')}.json"
...
def _load_pipeline_config(path: str | Path) -> PipelineConfig:
    ...
    llm_raw = raw.get("llm")
    stage_raw = raw.get("stage_5_intelligence")
    ...
    if not isinstance(stage_raw, dict):
        raise ConfigError("stage_5_intelligence must be a mapping")
    ...
    stage = Stage5IntelligenceConfig(
        max_items_default=_parse_non_negative_int(
            stage_raw.get("max_items_default"),
            "stage_5_intelligence.max_items_default",
        ),
        input_max_chars=_parse_positive_int(
            stage_raw.get("input_max_chars"),
            "stage_5_intelligence.input_max_chars",
        ),
    )
    return PipelineConfig(paths=paths, llm=llm, stage_5_intelligence=stage)
...
def _validate_report_invariants(payload: dict[str, Any]) -> None:
    ...
    if run_status not in {"completed", "fatal"}:
        raise FatalIntelligenceError("report invariant failed: run_status")
    if run_status == "completed" and fatal_error is not None:
        raise FatalIntelligenceError("report invariant failed: completed run must have fatal_error null")
    if run_status == "fatal" and (not isinstance(fatal_error, str) or not fatal_error.strip()):
        raise FatalIntelligenceError("report invariant failed: fatal run must have non-empty fatal_error")

    fail_breakdown = payload.get("fail_breakdown")
    if not isinstance(fail_breakdown, dict):
        raise FatalIntelligenceError("report invariant failed: fail_breakdown must be a map")
    if set(fail_breakdown.keys()) != set(FAIL_BREAKDOWN_KEYS):
        raise FatalIntelligenceError("report invariant failed: fail_breakdown keys mismatch")
    ...
    for field in int_fields:
        ...
        if not is_non_bool_int(value) or int(value) < 0:
            raise FatalIntelligenceError(f"report invariant failed: {field} must be integer >= 0")
    for key in FAIL_BREAKDOWN_KEYS:
        value = fail_breakdown.get(key)
        if not is_non_bool_int(value) or int(value) < 0:
            raise FatalIntelligenceError(f"report invariant failed: fail_breakdown[{key}] invalid")
    ...
    for field in (
        ...
        if not isinstance(payload.get(field), str):
            raise FatalIntelligenceError(f"report invariant failed: {field} must be a string")
    ...
    if items_selected != success_count + failed_count:
        raise FatalIntelligenceError("report invariant failed: items_selected mismatch")
    if success_count != inserted_db + skipped:
        raise FatalIntelligenceError("report invariant failed: success_count mismatch")
    if failed_count != fail_total:
        raise FatalIntelligenceError("report invariant failed: failed_count mismatch")
    if items_selected > selected_rows_total:
        raise FatalIntelligenceError("report invariant failed: items_selected > selected_rows_total")
    if selected_rows_total > max_items:
        raise FatalIntelligenceError("report invariant failed: selected_rows_total > max_items")
    if items_selected > items_available_total:
        raise FatalIntelligenceError("report invariant failed: items_selected > items_available_total")
    if run_status == "completed" and items_selected != selected_rows_total:
        raise FatalIntelligenceError("report invariant failed: completed run items_selected mismatch")
